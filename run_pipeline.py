...
def _youtube_cookie_file(job_dir: Path) -> Path | None:
    """Create a temporary Netscape cookies file from Railway env vars.

    Preferred: YOUTUBE_COOKIES_B64 (base64-encoded cookies.txt)
    Fallback:  YOUTUBE_COOKIES (raw Netscape cookies.txt content)
    """
    raw_b64 = (os.environ.get("YOUTUBE_COOKIES_B64") or "").strip()
    raw = os.environ.get("YOUTUBE_COOKIES") or ""
    if not raw_b64 and not raw.strip():
        return None

    cookie_path = job_dir / "youtube_cookies.txt"
    try:
        if raw_b64:
        ...
        raise RuntimeError(f"Invalid YouTube cookies variable: {e}") from e

    text = cookie_path.read_text(encoding="utf-8", errors="replace")
    if not (text.startswith("# Netscape HTTP Cookie File") or text.startswith("# HTTP Cookie File")):
        raise RuntimeError("YouTube cookies must be exported in Netscape cookies.txt format")
    return cookie_path


def download_video(url: str, job_dir: Path) -> Tuple[Path, str, Path | None]:
    cookie_file = _youtube_cookie_file(job_dir)
    ...
    if cookie_file:
        common_opts["cookiefile"] = str(cookie_file)

    probe_opts = {**common_opts, "skip_download": True}
    ...
    opts: Dict[str, Any] = {
        **common_opts,
        ...
        "outtmpl": outtmpl,
        # A small delay reduces repeated anti-bot/rate-limit triggers on hosted IPs.
