...
# V2: shared token is embedded so Railway does not depend on WORKER_API_TOKEN.
# You can rotate it later by replacing this value in both worker_api.py and n8n.
SHARED_TOKEN = "ytw2_kglwDTvMKs9M2Py65_7FglY4d7w9DylDZhAd6XhcQxg"

app = FastAPI(title="YouTube Shorts Worker", version="2.0.0")
...
def _check_token(x_worker_token: str | None) -> None:
    if not x_worker_token or not secrets.compare_digest(x_worker_token.strip(), SHARED_TOKEN):
...
def root() -> dict:
    return {
        "ok": True,
        "service": "YouTube Shorts Worker",
        "version": "2.0.0",
        "kie_configured": bool(os.environ.get("KIE_API_KEY")),
...
def process_video(payload: ProcessRequest, x_worker_token: str | None = Header(default=None)) -> dict:
    ...
    if not pipeline.exists():
        raise HTTPException(status_code=500, detail="run_pipeline.py is missing from the container")
